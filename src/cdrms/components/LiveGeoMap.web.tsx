...
import type { LiveGeoMapProps } from './LiveGeoMap.native';
...
function embedUrl(latitude: number, longitude: number): string {
  const key = (process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY || '').trim();
  if (key) {
    return (
      `https://www.google.com/maps/embed/v1/place` +
      `?key=${encodeURIComponent(key)}` +
      `&q=${latitude}%2C${longitude}&zoom=18&maptype=roadmap`
    );
  }
  return `https://maps.google.com/maps?q=${latitude}%2C${longitude}&z=18&hl=en&output=embed`;
}
...
export function LiveGeoMap({
  ...
  const src = embedUrl(latitude, longitude);
  return (
